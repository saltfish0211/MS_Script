...
function monsterValue(eim, mobId, player) {
    if(mobId == 8880343 || mobId == 8880311 || mobId == 8880312){
...
function nextMap(eim){
    if (em.getProperty("state").equals("2")==true){
        var mapA = eim.getMapInstance(450008250);
        ...
        var mapA = eim.getMapInstance(450008350);
        eim.schedule("monsterSpawn", 1);
        for (var i = 0; i < eim.getPlayerCount() ; i++) {
            eim.getPlayers().get(i).changeMap(mapA, mapA.getPortal(0))
        }
...
function monsterSpawn(eim) {
    var map = eim.getMapInstance(450008150);
    var map1 = eim.getMapInstance(450008250);
    var map2 = eim.getMapInstance(450008350);
    ...
    if (em.getProperty("state").equals("1")==true){
        ...
        //em.getMapFactory().getMap(450008750).startSimpleMapEffect("1分鐘中後將關閉次元世界，請盡快領取點擊NPC領取獎勵後退出",5120093);
    }
}
