...
var EventConfig = BossEventConfigs.卡翁;
...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("BossKawon");
    var map = eim.setInstanceMap(221030910); //設置活動腳本的地圖
...
function playerRevive(eim, player) {
    ...
    player.addHP(50);
    var map = eim.getMapFactoryMap(221030900);
    player.changeMap(map, map.getPortal(0));
    return true;
}
...
function monsterKilled(eim, player, cp) {
    if (cp == 8880200) {
        player.getMap().killMonster(8880202);
    }
}
