...
function setup(eim, leaderid) {
    ...
    var eim = em.newInstance("BossNouhime");
    var map = eim.setInstanceMap(811000500);
    map.resetFully();
...
function playerRevive(eim, player) {
    ...
    player.addHP(50);
    var map = eim.getMapFactoryMap(811000999);
    player.changeMap(map, map.getPortal(0));
    return true;
}
...
function end(eim) {
    if (eim.disposeIfPlayerBelow(100, 811000999)) {
        eim.dispose();
        em.setProperty("state", "0");
        em.setProperty("leader", "true");
    }
