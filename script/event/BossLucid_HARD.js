...
function setup(level, leaderid) {
    ...
    eim.schedule("checkLucidPosition", 5000); 
...
function checkLucidPosition(eim){
    eim.schedule("checkLucidPosition", 5000); 
    if(em.getProperty("state") != "2"){
        return;
    }
    var map = eim.getMapInstance(450004550);
    for each (mob in map.getMonsters()) {
        if (mob.getId() == 8880151) {
            
            if(mob.getPosition().getX() <= -100 || mob.getPosition().getX() >= 1500
            || mob.getPosition().getY() >= 100 || mob.getPosition().getY() <= -900){
                map.moveMonster(mob,new java.awt.Point(657, -490))
            }
...
function 階段1(eim){
    ...
    eim.schedule("刷新石頭", 15000); 
    eim.schedule("刷新蘑菇", 10000); 
    eim.schedule("刷新蝴蝶", 10000);     
...
function 階段3(eim){
    ...
    map = eim.setInstanceMap(450004850);
    map.killAllMonsters(false);
    for each(monsterStatus in EventConfig.Monsters[450004850]) {
        var mob = getMonster(monsterStatus, em);       
        if(em.getProperty("practice") == "true"){
            mob.disableDrops();
        }    
