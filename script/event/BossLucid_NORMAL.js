...
function changedMap(eim, player, mapid) {
    ...
    if( mapid == 450004100 && eim.isLeader(player)){
        eim.schedule("階段1", 0);
    }
...
function 階段1(eim){
    ...
    var map = eim.setInstanceMap(450004100);
    for each(monsterStatus in EventConfig.Monsters[450004100]) {
...
function 完成階段(eim){
    ...
    map = eim.setInstanceMap(450004200);
    for each(monsterStatus in EventConfig.Monsters[450004200]) {
        var mob = getMonster(monsterStatus, em);     
        if(em.getProperty("practice") == "true"){
            mob.disableDrops();
        }   
...
function 階段2(eim){
    ...
    map = eim.setInstanceMap(450004250);
    for each(monsterStatus in EventConfig.Monsters[450004250]) {
        var mob = getMonster(monsterStatus, em);   
        if(em.getProperty("practice") == "true"){
            mob.disableDrops();
        }     
...
function 階段1結束(eim){
    var mapA = eim.getMapInstance(450004250);
    for (var i = 0; i < eim.getPlayerCount() ; i++) {
...
function 階段2結束(eim){
    var mapA = eim.getMapInstance(450004200);
    for (var i = 0; i < eim.getPlayerCount() ; i++) {
        eim.getPlayers().get(i).changeMap(mapA, mapA.getPortal(0))
    }
}