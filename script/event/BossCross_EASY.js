...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("BossCross_EASY");
    var map = eim.setInstanceMap(806300200); //設置活動腳本的地圖
    map.resetFully(false); //重置地圖
    map.setSpawns(false);
    //載入BOSS自訂數值        
    BossStatu = Boss("Cross_easy",em);
...
function scheduledTimeout(eim) {
    eim.disposeIfPlayerBelow(100, 806300100);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
}
...
function playerRevive(eim, player) {
    ...
    player.addHP(50);
    var map = eim.getMapFactoryMap(806300100);
    player.changeMap(map, map.getPortal(0));
    return true;
}
