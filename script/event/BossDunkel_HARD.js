...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("BossDunkel_HARD");
    var map = eim.setInstanceMap(450012600); //設置活動腳本的地圖
...
function addFirstMonsters(d) {
    ...
    var h = f.nextInt(3) + 2;
    d.schedule('addFirstMonsters', 1000 * 5 * h);
}

function summonMob(eim) {
    var b;
    b = eim.getMapInstance(450012600);
    var m = [
        [8220022,-577]
        ,[8220023,-319]
        ,[8220024,45]
    ...
    var num = 0;
    num += b.countMonsterById(8220022);
    num += b.countMonsterById(8220023);
    num += b.countMonsterById(8220024);
    num += b.countMonsterById(8220025);
    num += b.countMonsterById(8220026);
    
    
    if (num < 40) {
        for(i=0;i<m.length;i++){
            var monster = em.getMonster(m[i][0]);
            var changeStats = monster.getForcedMobStat();
            if (changeStats == null) {
                monster.setForcedMobStat(monster.getStats());
                changeStats = monster.getForcedMobStat();
            }
            monster.changeHP(800000000);
            changeStats.setWatk(10000);
            changeStats.setMatk(10000);
            changeStats.setAcc(500000);
            changeStats.setSpeed(300);
            ...
            var b = eim.getMapInstance(450012600);
            b.spawnMonsterOnGroundBelow(monster, new java.awt.Point(m[i][1], 29));
        }
    }
    if (em.getProperty('ZhaoHuan')==1) {
        eim.schedule('summonMob', 1000 * 20);
    }
}
...
function monsterKilled(eim, player, cp) {
    if (cp == 8645066) {
        player.getMap().killMonster(8645068);
    }
}
