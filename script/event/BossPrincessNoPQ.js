...
function setup(level, leaderid) {
    ...
    mob.changeLevel(180);
    mob.changeHP(200000000000);
    // mob.setHp(200000000000);
...
function stage1Check(eim) {
    if (eim.getProperty("stage1").equals("0")) {
        var map = eim.getMapInstance(0);
        var mob = em.getMonster(9450004);//水鏡
        eim.registerMonster(mob);
        eim.setProperty("stage1", "1");//召喚小BOSS
...
function stage2Check(eim) {
    ...
    if (!eim.getProperty("stage2").equals("4")) {
        ...
        var mob = em.getMonster(9450010);//蓮華
        eim.registerMonster(mob);
        eim.setProperty("stage2", "5");//召喚小BOSS
...
function stage3KillMob(eim) {
    var map = eim.getMapInstance(2);
    var count = map.countMonsterById(9450012); //根據存活的數量來召喚出小BOSS

    map.killMonster(9450011);
    map.killMonster(9450043);
    map.killMonster(9450044);
    map.killMonster(9450045);
    map.killMonster(9450046);
    map.killMonster(9450013);

    var mob = em.getMonster(9450014);//翁羅將軍
    eim.registerMonster(mob);
    eim.setProperty("stage3", "2");//召喚小BOSS
...
function stage4Check(eim) {
    if (eim.getProperty("stage4").equals("0")) {
        ...
        var mob = em.getMonster(9450020);//彌弄矩
        eim.registerMonster(mob);
        eim.setProperty("stage4", "1");//召喚小BOSS
