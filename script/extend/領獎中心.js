...
function start() {
    ...
    for (var i = 0; i < rewardArray.length; i++) {
        ...
        totalOnlineTime = cm.getOnlineTime();
    ...
    if (status == 0) {
        ...
        switch(rewardMode){
            ...
            case 3:
                ...
                var text = "#b#e不可領取的獎勵 "+ maskRewardTitle(selectedReward, true) +"#b#e，可確認獎勵內容#k\r\n";
                for (var i = 0; i < selectedReward.reward.items.length; i++) {
                    var item = selectedReward.reward.items[i];
                    text += maskItem(item.id) + "x" + item.count + "\r\n";
                    text += maskItemDescription(item);
...
function isAllowReward(reward) {
    ...
    if ((reward.requiredTime != null && totalOnlineTime < reward.requiredTime) || isNotAllowOnlineReward(reward.requiredTime)) {
        canTake = false;
...
function isNotAllowOnlineReward(time){
    if(time == null) return false;
    return (hour == 23 && (minute >= 50 && minute <= 59)) || (hour == 0 && (minute >= 0 && minute <= 10));
}
