...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            if(cm.getSpace(5) < 2){
                   cm.sendOk("#b特殊#k包袱空間不足#r2#k個,請整理後在使用此功能");
                cm.dispose();
                return;
            }else{
            cm.sendSimple("我這裡可以製作寵物！不定期刪除和更新最新寵物。 #n\r\n\r\n#L0##r" + aaa + "免費獲得絕版神寵。#k#l\r\n#L1##b" + aaa + "免費獲得寵物機器人.#l\r\n#L2##b" + aaa + "復活我寶貝可愛的寵物#k#l\r\n\r\n#d注意:您的寵物需要15級\r\n#d請不要把寵物放進商場裡不然會有可能取不回來哦#b\r\n在商城取不下來的寵物請選擇他右鍵刪除");
}
        } else if (status == 1) {
            if (selection == 0) {
                var currentpet = null;
                ...
                if (pet == null || !cm.haveItem(5380000, 1)) {
                    ...
                    cm.sendOk("你的寵物#i" + id + "##t" + id + "#,已經進化成#i" + after + "##t" + after + "#!");
                    cm.dispose();
                }
            } else if (selection == 1) {
                var currentpet = null;
