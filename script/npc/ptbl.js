...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == startmap) {
        if (status == 0) {
            ...
            rwpz += "    剩余挑战次数：#r#e" + (maxenter[selection] - cm.getDaysPQLog(PQLog[selection], interval[chs])) + "#n#k 次#n#k\r\n\r\n";
            ...
            if (cm.getParty() == null) { //判断组队
            ...
            } else if (!cm.isAllPartyMembersAllowedPQ(PQLog[chs], maxenter[chs], interval[chs])) {
                cm.sendNext("你的队员#r#e \"" + cm.getNotAllowedPQMemberName(PQLog[chs], maxenter[chs], interval[chs]) + "\" #k#n次数已经达到上限了。");
            } else if (!cm.allMembersHere()) {
                cm.sendOk("你的组队部分成员不在当前地图,请召集他们过来后在尝试。"); //判断组队成员是否在一张地图..
                 } else if (!cm.getPlayerCount(105200410) == 0) {
                ...
                if (em == null || open == false) {
                    ...
                    if (prop == null || prop.equals("0")) {
                        ...
                        cm.worldSpouseMessage(0x0F, "[副本挑战] : 恭喜 " + cm.getChar().getName() + " 开始挑战普通贝伦。祝Ta满载而归！");
                    } else {
